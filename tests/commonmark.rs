...
use markdown::{
    message, to_html_with_options as canonical_to_html, CompileOptions, Options, Renderer,
};
use pretty_assertions::assert_eq;

fn to_html_with_options(value: &str, options: &Options) -> Result<String, message::Message> {
    let canonical = canonical_to_html(value, options)?;
    let renderer = Renderer::open(
        value,
        Options {
            compile: options.compile.clone(),
            ..Options::default()
        },
    )?;
    assert_eq!(renderer.segmented_html(), canonical);
    Ok(canonical)
}
