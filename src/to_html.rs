...
use crate::util::{
    ...
    encode::{encode, encode_into},
    gfm_tagfilter::gfm_tagfilter,
    infer::{gfm_table_align, list_loose},
    normalize_identifier::{normalize_identifier, normalize_identifier_into},
...
use core::{convert::TryFrom, ops::Range, str};

/// HTML produced by one canonical compilation, plus output ranges for the
/// requested top-level source ranges.
pub(crate) struct SegmentedHtml {
    pub(crate) html: String,
    pub(crate) output_ranges: Vec<Range<usize>>,
}
...
struct CompileContext<'a> {
    ...
    definition_ids: BTreeMap<String, DefinitionId>,
    /// Reusable normalized identifier lookup storage.
    identifier_scratch: String,
    /// List of definitions.
...
impl<'a> CompileContext<'a> {
    ...
    fn new(
        ...
        CompileContext {
            ...
            buffers: vec![String::with_capacity(bytes.len())],
            index: 0,
            options,
        }
    }
    ...
    fn push_encoded(&mut self, value: &str, encode_html: bool) {
        let last = self
            .buffers
            .last_mut()
            .expect("at least one buffer should exist");
        encode_into(value, encode_html, last);
    }

    /// Add a line ending.
    fn line_ending(&mut self) {
        match self.line_ending_default {
            LineEnding::CarriageReturnLineFeed => self.push("\r\n"),
            LineEnding::CarriageReturn => self.push("\r"),
            LineEnding::LineFeed => self.push("\n"),
        }
    }
...
pub fn compile(events: &[Event], bytes: &[u8], options: &CompileOptions) -> String {
    compile_internal(events, bytes, options, &[]).html
}

/// Compile canonical HTML and locate the output generated by each ordered,
/// non-overlapping top-level source range.
pub(crate) fn compile_segmented(
    events: &[Event],
    bytes: &[u8],
    options: &CompileOptions,
    source_ranges: &[Range<usize>],
) -> SegmentedHtml {
    compile_internal(events, bytes, options, source_ranges)
}
...
fn compile_internal(
    ...
    let mut output_ranges = Vec::with_capacity(source_ranges.len());
    ...
    while index < events.len() {
        if source_index < source_ranges.len()
            && output_end.is_some()
            && events[index].point.index > source_ranges[source_index].end
        {
            output_ranges.push(output_start.take().unwrap_or(0)..output_end.take().unwrap_or(0));
            source_index += 1;
        }
        if output_start.is_none()
            && source_index < source_ranges.len()
            && events[index].point.index >= source_ranges[source_index].start
        {
            output_start = Some(context.buffers.first().map_or(0, String::len));
            if events[index].point.index >= source_ranges[source_index].end {
                let offset = output_start.take().unwrap_or(0);
                output_ranges.push(offset..offset);
                source_index += 1;
            }
        }
        if index == jump.0 {
            while source_index < source_ranges.len()
                && source_ranges[source_index].start >= events[jump.0].point.index
                && source_ranges[source_index].end <= events[jump.1].point.index
            {
                let offset = context.buffers.first().map_or(0, String::len);
                output_ranges.push(offset..offset);
                output_start = None;
                output_end = None;
                source_index += 1;
            ...
            handle(&mut context, index);
            if source_index < source_ranges.len()
                && events[index].kind == Kind::Exit
                && events[index].point.index == source_ranges[source_index].end
                && matching_enter_start(events, index) == source_ranges[source_index].start
            {
                output_end = Some(context.buffers.first().map_or(0, String::len));
            }
            index += 1;
        }
    }
    ...
    let html = context.buffers.pop().expect("expected 1 final buffer");
    if source_index < source_ranges.len() {
        if let Some(end) = output_end.take() {
            output_ranges.push(output_start.take().unwrap_or(end)..end);
            source_index += 1;
        }
    }
    while source_index < source_ranges.len() {
...
fn matching_enter_start(events: &[Event], exit_index: usize) -> usize {
    let name = &events[exit_index].name;
    let mut depth = 1;
    let mut index = exit_index;
    while index > 0 {
        index -= 1;
        ...
        match events[index].kind {
            ...
            Kind::Enter => {
                depth -= 1;
                if depth == 0 {
                    return events[index].point.index;
                }
            }
        }
    }
    events[exit_index].point.index
}
...
fn on_exit_character_reference_value(context: &mut CompileContext) {
    ...
    context.push_encoded(&value, context.encode_html);
}
...
fn on_exit_data(context: &mut CompileContext) {
    let indices = Position::from_exit_event(context.events, context.index).to_indices();
    ...
    if skip_marker {
        let slice = Slice::from_indices(context.bytes, indices.0, indices.1);
        let text = slice.as_str();
        ...
        if !rest.is_empty() {
            let output = context
                ...
                .last_mut()
                .expect("at least one buffer should exist");
            encode_into(rest, context.encode_html, output);
        }
        return;
    }
    ...
    let output = context
        ...
        .last_mut()
        .expect("at least one buffer should exist");
    encode_into(value, context.encode_html, output);
}
...
fn on_exit_definition(context: &mut CompileContext) {
    ...
    if !context.definition_ids.contains_key(id.as_str()) {
        let definition_id = DefinitionId(
            u32::try_from(context.definitions.len()).expect("too many Markdown definitions"),
        );
        context.definition_ids.insert(id, definition_id);
...
fn on_exit_media(context: &mut CompileContext) {
    ...
    let definition_index = if media.destination.is_none() {
        let indices = media
            .reference_id
            .or(media.label_id)
            .expect("reference media has an identifier");
        normalize_identifier_into(
            Slice::from_indices(context.bytes, indices.0, indices.1).as_str(),
            &mut context.identifier_scratch,
        );
        let definition_id = context
            .definition_ids
            .get(context.identifier_scratch.as_str())
            .expect("expected defined definition");
        Some(definition_id.0 as usize)
    } else {
        None
    };
